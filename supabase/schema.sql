...
create table if not exists lead_internet_observations (
  ...
  identity_score integer check (identity_score between 0 and 100), match_reasons jsonb not null default '[]'::jsonb,
  evidence_type text not null default 'single_source' check (evidence_type in ('verified','single_source')),
  published_at timestamptz, corroboration_count integer not null default 1 check (corroboration_count >= 1),
