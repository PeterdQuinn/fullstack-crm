...
import { retryAutomationDb } from "./automation-db-retry";
...
export async function withAutomationRun(stage: string, req: NextRequest, run: () => Promise<Response>) {
  ...
  const { data: settings, error: settingsError } = await retryAutomationDb(() => db.from("automation_settings").select("enabled").eq("id", "owner").single());
  ...
  const { error } = await retryAutomationDb(() => db.from("automation_runs").upsert({ id: record.id, stage }, { onConflict: "id", ignoreDuplicates: true }));
