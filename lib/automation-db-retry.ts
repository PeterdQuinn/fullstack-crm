export function isTransientAutomationDbError(result: { error: { message: string } | null; status?: number }): boolean {
...
export async function retryAutomationDb<T extends { error: { message: string; code?: string } | null; status?: number }>(
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    const transient = isTransientAutomationDbError(result);
